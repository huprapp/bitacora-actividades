...
const SETTINGS_PASSWORD_DEFAULT = "social2025"; // puedes cambiarlo en .env
...
export default function App() {
  ...
  const [expanded, setExpanded] = useState({}); // filas expandibles
  ...
  const exportCSVEntries = () => {
    const rows = [];
    rows.push(["Nombre del Responsable", "Fecha (seleccionada)", "Registrado (PR)", "Actividad", "Descripción", "Cantidad", "Tipo", "Notas"]);
    for (const e of entries) {
      for (const t of DEFAULT_TASKS) {
        const te = e.tasks?.[t.key] || { description: "", quantity: "" };
        const qty = parseN(te.quantity);
        if (qty > 0 || te.description) rows.push([e.personName, e.date, fmtDateTimePR(e.createdAt), t.label, (te.description || "").replace(/\n/g, "; "), String(qty), "Base", (e.notes||"").replace(/\n/g, "; ")]);
      }
      for (const o of e.otros || []) {
        const qty = parseN(o.quantity);
        if (qty > 0 || o.description) rows.push([e.personName, e.date, fmtDateTimePR(e.createdAt), o.label || "Otros", (o.description || "").replace(/\n/g, "; "), String(qty), "Otros", (e.notes||"").replace(/\n/g, "; ")]);
      }
    }
                      ...
                      {entries.slice().reverse().map((e) => (
                        <div key={e.id} className="border rounded-xl p-3 bg-white/90">
                          <div className="grid grid-cols-12 gap-2 items-center">
                            ...
                            <div className="col-span-6 md:col-span-2 text-sm">Fecha: {e.date || '—'}</div>
                            <div className="col-span-6 md:col-span-3 text-sm">Registrado (PR): {fmtDateTimePR(e.createdAt) || '—'}</div>
                            <div className="col-span-12 md:col-span-3 text-sm">Total: {e.total ?? (DEFAULT_TASKS.reduce((s, t) => s + parseN(e.tasks?.[t.key]?.quantity), 0) + (e.otros || []).reduce((s, o) => s + parseN(o.quantity), 0))}</div>
